...
rf_regressor = joblib.load("rf_model_white.joblib")

d_white = pd.read_csv('winequality-white.csv', delimiter=';',header=0)
from sklearn.impute import SimpleImputer
# imputer to replace any Nan or Null values
imputer = SimpleImputer(strategy='mean')
col=d_white.columns #storing the columns of the dataframe as imputer removes it
d_white=pd.DataFrame(imputer.fit_transform(d_white))
d_white.columns=col
from sklearn.preprocessing import MinMaxScaler

# Normalizing the data
scaler = MinMaxScaler()
d_white.iloc[:,:-1] = scaler.fit_transform(d_white.iloc[:,:-1])
...
def main():
    ...
    <h2 style="color:white;text-align:center;">White wine random forest regressor</h2>
    </div>
    """
    ...
    ca = st.select_slider("citric acid",np.arange(0, 2, 0.1))
    rs = st.select_slider("residual sugar",np.arange(0, 20, 0.1))
    chlo=st.select_slider("chlorides",np.arange(0, 1, 0.1))
    fsd=st.select_slider("free sulphur dioxide",np.arange(0, 100, 1))
    tsd=st.select_slider("total sulphur dioxide",np.arange(0, 300, 1))
    dens=st.select_slider("density",np.arange(0.9, 1.1, 0.0001))
